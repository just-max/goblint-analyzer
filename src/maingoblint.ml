...
let check_arguments () =
  ...
  if get_bool "incremental.load" && CompareCIL.enabled_comparisons () = [] then warn "incremental.load is enabled, but no comparison strategies are selected. All functions will be detected as changed."
